...
# Note: in the protein-receptor complex structure, the residue name of the receptor is 'BGLCNA', 'BGAL' and 'ANE5AC'
...
def HydrogenBondDetection(Environ,frame_index):
	...
	# Hbond donor and acceptor selection
	hbonds = HBA(universe=Environ,between=["resname BGLCNA or resname BGAL or resname ANE5AC", "protein"])
	protein_hydrogens_sel = hbonds.guess_hydrogens("protein")
	protein_acceptors_sel = hbonds.guess_acceptors("protein")

	LIG_hydrogens_sel = hbonds.guess_hydrogens("resname BGLCNA or resname BGAL or resname ANE5AC")
	LIG_acceptors_sel = hbonds.guess_acceptors("resname BGLCNA or resname BGAL or resname ANE5AC")	
